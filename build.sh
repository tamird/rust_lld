...
rustc --emit obj $FILE.rs -o target/$FILE.o && ld -demangle -dynamic -arch x86_64 -o target/$FILE -L$RUSTLIB target/$FILE.o -force_load $RUSTLIB/libmorestack.a -dead_strip -l$(find $RUSTLIB -name libstd-*.dylib | head -n1 | sed s,$RUSTLIB/lib,, | sed s,\.dylib$,,) -lSystem -lpthread -lc -lm -rpath @loader_path/../../rust-build/x86_64-apple-darwin/stage2/lib/rustlib/x86_64-apple-darwin/lib -rpath /usr/local/lib/rustlib/x86_64-apple-darwin/lib -lcompiler-rt && DYLD_LIBRARY_PATH=$RUSTLIB target/$FILE
clean && echo
print 'Static linking: running `rustc` and then `ld`...'
rustc --emit obj $FILE.rs -o target/$FILE.o && ld -demangle -dynamic -arch x86_64 -o target/$FILE -L$RUSTLIB target/$FILE.o -force_load $RUSTLIB/libmorestack.a -dead_strip $RUSTLIB/lib{core,std,alloc,collections,rustc_unicode}*.rlib -lSystem -lpthread -lc -lm -lcompiler-rt && target/$FILE
clean && echo
